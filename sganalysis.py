...
class SGA:
    ...
    def segment_cells(self, img):
        ...
        mask, flows, styles, diams = model.eval(
            img, diameter=150, flow_threshold=None, channels=self.cell_channels
        )
    ...
    def segment_nuclei(self, img):
        ...
        mask, flows, styles, diams = model.eval(
            img, diameter=80, flow_threshold=None, channels=self.nuclei_channels
        )
        return mask.astype(np.uint)
    ...
    def measure_objects_in_cell(self, roi, img, cells, nuclei, granules, other):
        ...
        keep = [
            ...
            if x.perimeter_crofton > 1 and x.area > 1 and x.major_axis_length > 1
        ]
        ...
        sum_particle = [
            np.sum(img[k, :, :] * mask_particle) for k in range(img.shape[0])
        ]
        ...
        if len(regions) > 0:
            D1 = {
                ...
                "Cell ID": id,
                "Mean intensity of granule": [
                    x.mean_intensity[self.granule_channel] for x in regions
                ],
                "Total intensity of granule": [
                    x.area * x.mean_intensity[self.granule_channel] for x in regions
                ],
                "Mean Intensity of granule ratio of particle:cytosol": [
                    x.mean_intensity[self.granule_channel]
                    / mean_cytosol[self.granule_channel]
                    for x in regions
                ],
                "Mean Intensity of other": [
                    x.mean_intensity[self.other_channel] for x in regions
                ],
                "Total Intensity of other": [
                    x.area * x.mean_intensity[self.other_channel] for x in regions
                ],
                "Mean Intensity of other ratio of particle:cytosol": [
                    x.mean_intensity[self.other_channel]
                    / mean_cytosol[self.other_channel]
                    for x in regions
                ],
                ...
                "Circularity": [
                    4.0 * math.pi * x.area / x.perimeter_crofton**2 for x in regions
                ],
                "Aspect ratio": [
                    x.minor_axis_length / x.major_axis_length for x in regions
                ],
                "Solidity": [x.solidity for x in regions],
                "Roundness": [
                    4.0 * x.area / (math.pi * x.major_axis_length**2) for x in regions
                ],
            }
        else:
            D1 = None
        ...
        m1, m2 = manders_coefficients(
            (granules > 0) * mask_cell, (other > 0) * mask_cell
        )
        D2 = {
            "Cell ID": id,
            ...
            "Mean intensity in cell of granule channel": mean_cell[
                self.granule_channel
            ],
            "Mean intensity in cell of other channel": mean_cell[self.other_channel],
            "Total Intensity in cell of granule channel": sum_cell[
                self.granule_channel
            ],
            "Total Intensity in cell of other channel": sum_cell[self.other_channel],
            "Mean intensity in cytosol of granule channel": mean_cytosol[
                self.granule_channel
            ],
            "Mean intensity in cytosol of other channel": mean_cytosol[
                self.other_channel
            ],
            "Total Intensity in cytosol of granule channel": sum_cytosol[
                self.granule_channel
            ],
            "Total Intensity in cytosol of other channel": sum_cytosol[
                self.other_channel
            ],
            "Mean intensity in particle of granule channel": mean_particle[
                self.granule_channel
            ],
            "Mean intensity in particle of other channel ": mean_particle[
                self.other_channel
            ],
            "Total Intensity in particle of granule channel": sum_particle[
                self.granule_channel
            ],
            "Total Intensity in particle of other channel": sum_particle[
                self.other_channel
            ],
            "Intensity ratio of other:granule in particle": mean_particle[
                self.other_channel
            ]
            / mean_particle[self.granule_channel],
            "Intensity ratio of other:granule in cell": mean_cell[self.other_channel]
            / mean_cell[self.granule_channel],
            "Intensity ratio of other:granule in cytosol": mean_cytosol[
                self.other_channel
            ]
            / mean_cytosol[self.granule_channel],
            "Mean intensity ratio of other particle:cytosol": mean_particle[
                self.other_channel
            ]
            / mean_cytosol[self.other_channel],
            "Mean intensity ratio of granule particle:cytosol": mean_particle[
                self.granule_channel
            ]
            / mean_cytosol[self.granule_channel],
            ...
            "Spread of particle": self.spatial_spread(
                mask_particle, img[self.granule_channel, :, :]
            ),
            "Mean particle area": np.mean(D1["Area"]) if D1 is not None else 0.0,
            "Mean particle perimeter": np.mean(D1["Perimeter"])
            if D1 is not None
            else 0.0,
            "Mean particle distance to nuclei": np.mean(D1["Distance to nuclei"])
            if D1 is not None
            else 0.0,
            "Mean particle distance to edge": np.mean(D1["Distance to edge"])
            if D1 is not None
            else 0.0,
            "Mean particle circularity": np.mean(D1["Circularity"])
            if D1 is not None
            else 0.0,
            "Mean particle aspect ratio": np.mean(D1["Aspect ratio"])
            if D1 is not None
            else 0.0,
            "Mean particle solidity": np.mean(D1["Solidity"])
            if D1 is not None
            else 0.0,
            "Mean particle roundness": np.mean(D1["Roundness"])
            if D1 is not None
            else 0.0,
        }
        if D1 is None:
    ...
    def measure(self, img, cells, nuclei, granules, other):
        ...
        for roi in rois:
            ...
            if (
                np.all(roi.coords > 0)
                and (np.all(roi.coords[:, 0] < img.shape[1] - 1))
                and (np.all(roi.coords[:, 1] < img.shape[2] - 1))
            ):
                o, r = self.measure_objects_in_cell(
                    roi, img, cells, nuclei, granules, other
                )
                if o is not None:
                    obj_df.append(o)
                    roi_df.append(r)
...
def main():
    ...
    parser.add_argument(
        "--output-by-granules", help="filename of the output table by granule"
    )
    parser.add_argument(
        "--output-by-cells", help="filename of the output table by cell"
    )
    parser.add_argument(
        "--output-cell-contours", help="filename of the output contours file"
    )
    parser.add_argument(
